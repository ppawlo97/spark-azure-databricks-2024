...
race_results_sdf = spark.read.parquet(RACE_RESULTS_FILE_PATH).filter(f"file_date = '{FILE_DATE}'")
...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.driver_standings"):
...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.constructor_standings"):
    final_constructors_sdf.write.mode("overwrite").insertInto(
