...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.results"):
    results_sdf_final.write.mode("overwrite").insertInto(
        f"{db_name}.results"
    )
...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.pit_stops"):
    pit_stops_sdf_final.write.mode("overwrite").insertInto(
        f"{db_name}.pit_stops"
    )
...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.lap_times"):
    lap_times_sdf_final.write.mode("overwrite").insertInto(
        f"{db_name}.lap_times"
    )
...
if spark._jsparkSession.catalog().tableExists(f"{db_name}.qualifying"):
    quali_sdf_final.write.mode("overwrite").insertInto(
